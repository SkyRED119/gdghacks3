 ...
 * OutlineUpload.tsx
 *
 * Lets a user upload a PDF course outline. The PDF is sent to Claude via the
 * Anthropic API which extracts assignments/deadlines and returns structured
 * JSON. Those assignments are then synced to the backend so the calendar
 * updates immediately.
 *
...
export function OutlineUpload({ onSynced }: Props) {
  ...
  async function handleFile(file: File) {
    ...
    // ── 2. Send to Claude to extract assignments
    setStage("parsing");
    ...
    try {
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        headers: {
          ...
          // Note: In production move this to your backend to keep the key safe.
          // For the hackathon, set VITE_ANTHROPIC_KEY in your .env.local
