...
function detectType(title: string): string {
  ...
  if (/final/i.test(t))                        return 'exam';      // Final Exam → exam
  if (/midterm|mid-term/i.test(t))   return 'midterm';   // Term Test → midterm
  if (/exam/i.test(t))                    return 'exam';
  if (/quiz|test/i.test(t))                         return 'quiz';
  if (/lab/i.test(t))                          return 'lab';
  if (/assignment|a\d+|hw|homework/i.test(t))  return 'assignment';
  return 'other';
}
...
function toCalendarEvent(a: any) {
  ...
  const isFinalExam = !dueDate && /final|final exam|final test|final assessment/i.test(title);
  ...
  if (dueDate) {
    ...
    const semesterStart = new Date(d.getFullYear(), Math.floor(calMonth / 4) * 4, 1);
    const day = semesterStart.getDay();
    const offset = day === 0 ? 1 : day === 1 ? 0 : 8 - day;
    semesterStart.setDate(semesterStart.getDate() + offset);
    
    const msPerWeek = 7 * 24 * 60 * 60 * 1000;
    week = Math.max(0, Math.floor((d.getTime() - semesterStart.getTime()) / msPerWeek));

  } else if (isFinalExam) {
    // No date but it's a final — put it in the last month of the semester (month index 3)
    // and last week (week 15 for Winter/Fall, 11 for Summer)
    month = 3;
    week = 15;
    isoDate = '';
  }
  ...
  return {
    ...
    weight: a.max_grade ?? 0,
    grade: a.grade ?? null,
    maxGrade: a.max_grade ?? null,
    type: a.type ?? detectType(a.title ?? ''),
  };
}
...
function toPlayerFromUser(user: any) {
  ...
  return {
    ...
    level,
    xp: user.xp ?? 0,
    xpMax: xpForLevel(level + 1),  // ← computed, matches extension formula
...
function AssignmentRow({ event, typeColours, delay = 0 }: { event: any; typeColours: Record<string, string>; delay?: number }) {
  ...
  const typeKey = (event.type ?? event.itemType ?? event.category ?? 'other').toLowerCase();
...
export default function App() {
  ...
  const [theme, setTheme] = useState<ThemeMode>(() => {
    return (localStorage.getItem('fl_theme') as ThemeMode) ?? 'dark';
  });
  ...
  const [typeColours, setTypeColours] = useState<Record<string, string>>(() => {
    try {
      const saved = localStorage.getItem('fl_typeColours');
      return saved ? { ...DEFAULT_TYPE_COLOURS, ...JSON.parse(saved) } : DEFAULT_TYPE_COLOURS;
    } catch {
      return DEFAULT_TYPE_COLOURS;
    }
  });
  ...
  return (
    <div data-theme={theme} className="flowestra-app">
          ...
          <div className="fl-page-header">
            <div>
              ...
              <div className="fl-stat-pills">
                ...
                <div className="fl-stat-pill"><strong>{events.filter(e => e.status === 'submitted').length}</strong> completed</div>
      ...
      {settingsOpen && (
        <SettingsPanel
          typeColours={typeColours}
                    onChangeColour={(t, v) => setTypeColours(prev => {
            const next = { ...prev, [t]: v };
            localStorage.setItem('fl_typeColours', JSON.stringify(next));
            return next;
          })}
      ...
      {/* ── Game modal ── */}
      {gameOpen && <GameModal onClose={() => setGameOpen(false)} isDark={theme === 'dark'} />}
    </div>
  );
}
